...
Table::Table() : minRow(102), minCol(102), maxRow(0), maxCol(0) {
	...
	cardBoard[52][52] = (std::shared_ptr<AnimalCard>)(AnimalCard*)new StartCard();
	board[52][52] = 1;
}
...
int Table::nbMatches(std::shared_ptr<AnimalCard> card, int row, int col) {
	...
	if (row-1>=0 && board[row - 1][col] == 1) {
		if ((row - 1 == 52 && col == 52) || cardBoard[row - 1][col]->getAnimal(2) == card->getAnimal(0) || cardBoard[row - 1][col]->getAnimal(3) == card->getAnimal(1))
			m++;
	}
	if (col-1 >= 0 && board[row][col-1] == 1) {
		if ((row == 52 && col - 1 == 52) || cardBoard[row][col-1]->getAnimal(1) == card->getAnimal(0) || cardBoard[row][col-1]->getAnimal(3) == card->getAnimal(2))
			m++;
	}
	if (col+1 < 103 && board[row][col+1] == 1) {
		if ((row == 52 && col + 1 == 52) || cardBoard[row][col+1]->getAnimal(0) == card->getAnimal(1) || cardBoard[row][col+1]->getAnimal(2) == card->getAnimal(3))
			m++;
	}
	if (row+1 < 103 && board[row + 1][col] == 1) {
		if ((row + 1 == 52 && col == 52) || cardBoard[row + 1][col]->getAnimal(0) == card->getAnimal(2) || cardBoard[row + 1][col]->getAnimal(1) == card->getAnimal(3))
			m++;
	}
	return m;
}
...
void Table::print() {
	...
	minCol = 102; minRow = 102; maxRow = 0; maxCol = 0;
...
bool Table::win(std::string& animal) {
	...
	for (int i = minRow; i != maxRow; i++) {
		for (int j = minCol; j != maxCol; j++) {
			if (board[i][j] != 0) {
				for (int k = 0; k != 4; k++) {
					if (std::string(1,charForAnimal(cardBoard[i][j]->getAnimal(k))) == animal) {
						nbSecretAnimalCard++;
						break;
					}
				}
